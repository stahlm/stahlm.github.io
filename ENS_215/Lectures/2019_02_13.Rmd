...
Today and in upcoming lectures you will learn some foundational concepts and technical aspects related to the analysis and statistical components of exploratory data analysis.  These tools and concepts will allow you extract greater information and understanding from your data and to make more rigorous and defensible statements related to your analysis.  
...
Ordinal data is similar to categorical except that there is an inherent ordering/ranking to the data.  For example:
  ...
  + Storm categories (e.g. Category 1, Category 2, Category 3,...)
...
FYI, we will learn later in the term, how to make more definitive and rigorous statements regarding the differences between sample means. 

<br/>

##### Quantiles
Quantiles are an important way of describing a dataset.  The $f$ quantile, $q(f)$, of a set of data is the **value** in a dataset such that a fraction $f$ of the data are less than or equal to that value (_Cleveland, 1994_).  For example, the 0.25 quantile (also referred to ast the 25^th^ percentile) is the value in a given dataset where 25% of the data are less than that value.  
...
+ Add a column called `quant` to the `ny_precip` data frame, which has the quantile for each precipitation value in the dataset.  **Note**: you can compute it relatively easily using the equation shown above or you can rely on a function in `dplyr` that will do it for you.  

You table should look similar the one below
```{r echo=FALSE, eval=TRUE}
ny_precip %>% 
  mutate(quant = cume_dist(Precip_inches))
...
ny_precip %>% 
  ...
  summarise(var_precip = var(Precip_inches), sd_precip = sd(Precip_inches))
```
