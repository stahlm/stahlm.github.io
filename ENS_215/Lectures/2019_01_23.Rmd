...
my_gap %>% filter(year == 2007) %>% 
  group_by(continent) %>% 
  summarise(gdp_percap_min = min(gdpPercap), gdp_percap_max = max(gdpPercap), gdp_percap_mean = mean(gdpPercap), life_min = min(lifeExp), life_max = max(lifeExp), life_mean = mean(lifeExp), num_country = n())
...
gap_summary_table <- my_gap %>% group_by(country) %>% 
  summarize(continent = first(continent), gdp_percap_ratio = (last(gdpPercap))/first(gdpPercap),gdp_ratio = (last(tot_gdp))/first(tot_gdp), life_exp_change = last(lifeExp) - first(lifeExp) ) %>% 
...
state_data %>% group_by(Month) %>% 
  summarize(m_avg = mean(Precip_inches), m_min = min(Precip_inches), m_max = max(Precip_inches)) 
...
t5 <- precip_data %>% group_by(state_cd, Year) %>% 
  summarize(tot_annual_precip = sum(Precip_inches)) %>% 
  group_by(state_cd) %>%  
  summarize(mean_precip = mean(tot_annual_precip)) %>% arrange(desc(mean_precip))
...
t6 <-  state_data %>% group_by(Year, season) %>% 
