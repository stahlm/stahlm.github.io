...
```{r}
ggplot(my_gap, aes(x = year, y = lifeExp)) + geom_line()
```
...
```{r}
ggplot(my_gap, aes(x = year, y = lifeExp, color = country)) + geom_line() +
...
```{r}
ggplot(my_gap, aes(x = year, y = lifeExp, group = country)) + geom_line()
```
...
```{r}
ggplot(my_gap, aes(x = year, y = lifeExp, group = country, color = continent)) + geom_line()
```
...
```{r}
ggplot(my_gap, aes(x = year, y = lifeExp, group = country, color = continent)) + geom_line(alpha = 0.4) + geom_smooth(aes(group = continent))
```
...
```{r}
my_gap %>% filter(continent == "Americas", year == 2007) %>% 
...
```{r}
my_gap %>% filter(continent == "Americas", year == 2007) %>% 
  ggplot(aes(x = lifeExp, y = reorder(country,lifeExp) )) + geom_point()
```
...
```{r}
ggplot(my_gap, aes(x = gdpPercap, y = lifeExp)) + geom_point()
```
...
```{r}
ggplot(my_gap, aes(x = gdpPercap, y = lifeExp)) + geom_point() + facet_wrap(~ continent)
```
...
my_gap %>% filter(year%in% c(1952, 1972, 1992, 2002)) %>% 
  ggplot(aes(x = gdpPercap, y = lifeExp)) + geom_point() + facet_grid(continent ~ year)
```
...
gap_americas <- my_gap %>% filter(continent == "Americas")

ggplot(gap_americas, aes(x = gdpPercap, y = lifeExp)) + 
  geom_point() + 
...
fig_americas <- ggplot(gap_americas, aes(x = gdpPercap, y = lifeExp)) + 
  geom_point() + 
  geom_vline(xintercept = 10000, color = "blue")
...
```{r}
fig_americas + geom_hline(yintercept = 65, color = "red")
```
...
```{r}
ggplot(life_exp_table, aes(x = life_1952, y = life_2007)) + geom_point()
```
...
```{r}
ggplot(life_exp_table, aes(x = life_1952, y = life_2007)) + geom_point() + geom_abline(slope = 1, intercept = 0, color = "red")
```
...
```{r eval=FALSE, echo=FALSE}
ggplot(life_exp_table, aes(x = life_1952, y = life_2007, color = continent)) + geom_point() + geom_abline(slope = 1, intercept = 0, color = "red") + facet_wrap(~ continent)
```
...
#### NOTE: When you get to this point please let me know before proceeding
...
```{r}
fig_gap <- ggplot(my_gap, aes(x = gdpPercap, y = lifeExp)) + geom_point() # let's save our "base/template" figure to an object so we can easily adapt it while typing less code
...
```{r}
fig_gap + xlim(0, 60000)
```

We can also change the y-limits  
```{r}
fig_gap + xlim(0, 60000) + ylim(30,80)
```
...
```{r}
As_fig <- ggplot(bangladesh_gw, aes(x = As_ugL, y = WELL_DEPTH_m)) + geom_point()
...
As_fig <- As_fig + scale_y_reverse() # reverse the y-axis and update As_fig
...
```{r}
As_fig <- As_fig + labs(title = "Groundwater arsenic in Bangladesh", 
