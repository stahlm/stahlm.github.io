...
In the lesson earlier this week, we learned how to use Q-Q plots to compare distributions between two sets of observations (e.g. monthly precipitation in NY and precipitation in RI).  We refer to those Q-Q plots as **empirical Q-Q plots**.  

We can also use Q-Q plots to compare the distribution of a dataset against a theoretical distribution.  For example we could compare the monthly precipitation in NY against a Normal distribution -- this would allows us to examine if the observed data (precip) is normally distributed and if it is not then how does it deviate from the theoretical distribution.  This type of Q-Q plot is referred as a **theoretical Q-Q plot**.  
...
If you find that your observed data fits a theoretical distribution, then the possibilities described above are generally available to you.  However, it is often the case that observed data will not 
...
+ Based on your Q-Q plot, does it seem as if the precipitation data is Normally distributed?  If not, where are the deviations from the Normal distribution most pronounced?  
...
+ Length of sentences
+ Incomes
+ Age of onset of certain diseases (_.e.g_ Alzheimers)
...
```{r message=FALSE}
flow <- read_csv("../Class_Data/USGS_gage_01335754.csv") %>% 
  drop_na()
```
...
```{r message=FALSE}
met_data <- read_csv("../Class_Data/HRECOS_Lock_8_windspeed_2016.csv")
