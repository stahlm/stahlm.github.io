...
Let's plot per capita GDP versus country in Asia for year 2007.  
```{r}
fig_1 <- my_gap %>% filter(year == 2007, continent == "Asia") %>% 
  ggplot(aes(x = reorder(country, gdpPercap), y = gdpPercap)) + geom_point()
...
```{r}
fig_1 <- fig_1 + theme(axis.text.x = element_text(angle = 60, hjust = 1))
...
```{r}
fig_1 + scale_y_continuous(labels=comma_format())
```
...
3. Remake any of the figures from today's lecture, though make the figure "presentation-quality"  
