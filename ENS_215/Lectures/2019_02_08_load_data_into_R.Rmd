...
Go to our class website and right-click the link below in today's notebook.  Select "_Save link as..._" and then save this file as "Mauna_Loa" in a sub-folder "Lecture_Data" within your "Lectures" folder for this class.  Make sure you save it as a csv file 
...
```{r}
CO2_data <- read_csv("./Lectures_Data/Mauna_Loa.csv")
```
...
```{r}
Mauna_loa <- read_csv("./Lectures_Data/Mauna_Loa_with_header.csv", skip = 4)
```
...
```{r}
Mauna_loa_another_example <- read_xlsx("./Lectures_Data/Mauna_Loa_excel_example.xlsx", range = "C8:D679")
```
