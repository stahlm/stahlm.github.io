...
In the environmental sciences (and in most other fields) data often has a spatial component.  Thus, you will commonly want to or need to generate maps to explore, analyze, and present your data.  

R has a wide-range of packages/functionality for performing spatial analysis and making maps.  In today's lesson we will learn some of the basics of map making in R.  Note that spatial analysis and map making fields unto their own and a whole course could easily be dedicated to this topic in R alone.  Nonetheless, using some of the basic map making functionality in R, you can learn how to make presentation-quality maps quickly and (relatively) easily.  For those interested in pursuing this topic further there are many excellent resources freely available.  The freely available book [Geocomputation with R](https://geocompr.robinlovelace.net/) is a great place to learn about this topic in greater detail.

Creating maps in R, also provides a huge advantage over many other map making tools -- all of your operations are clearly documented in your code and a map can be perfectly reproduced by simply re-running the code.  In many other map making software, many of the operations you perform are "point-and-click", thus there is often no easy way to reproduce your exact map from scratch and there is little record of how you did what you did. 
...
Look at that, you've create a map of the US!  
...
Take a quick look at this dataset to refamiliarize yourself with it.  You'll see that we will need to generate a summary of this data frame to get the average temperature for each US state.  
...
```{r}
state_name_table = tibble(state_name = tolower(state.name), state_cd = state.abb, us_region = state.region)
```
...
```{r}
state_mean_temps <- state_mean_temps %>% rename(region = state_name)
```
...
earthquakes <- read_csv("https://stahlm.github.io/ENS_215/Data/4.5_month_earthquakes_USGS.csv")
head(earthquakes)
```
