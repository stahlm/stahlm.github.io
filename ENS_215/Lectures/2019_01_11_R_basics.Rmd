...
+ Spend a few minutes testing out the different Markdown formatting options you learned above
...
Let's remove the NAs from `a_missing` and assign the new data to a new object called `a_cleaned`
```{r}
a_cleaned <- na.omit(a_missing) 
```

Look at `a_cleaned` in your Environment pane.  Does it look like everything worked?  
...
```
Notice that when using AND for a value of TRUE to be returned all of the conditions must be TRUE.   
...
The NOT operator is implemented in R using `!`.  This changes the truth of an evaluated statement.
```{r}
a <- 5 
b <- 3
c <- 10
...
We can also apply these operations on an element-by-element basis to a vector.  A vector of logical values (i.e. TRUE or FALSE) is returned.  Predict what the results will be before you run the code.
...
### Final exercise
If you have any remaining time, practice some more with the skills/tools you learned today.  
