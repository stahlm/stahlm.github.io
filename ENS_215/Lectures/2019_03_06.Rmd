...
You have been using functions in R since the very first day of this class.  The functions you have been using have been functions from base R or from one of many packages that we've been using.  Functions are great as they allow you to automate tasks that you commonly need to do.  Think about one of the most basic functions `mean()` -- this allows you to take the mean of a vector without having to compute the sum and then divide that sum by the number of values in the dataset.  More complex functions such as `lm()`, which computes a linear model, allow you to perform operations that take many lines of code in just a single function call.  

While, base R and the thousands of available R packages provide a wide variety of functions, many times you will find yourself needing a function that does not exist.  You can write your own R functions to automate tasks that frequently find yourself performing.  Writing a function is a much, much better approach than simply copying and pasting code to repeat tasks.  The benefits of function writing over the copy-and-paste approach is nicely summarized in the R4DS (excerpt below):
...
So if you find yourself copying and pasting code to perform the same task repeatedly, you should stop and write function.  
...
You can see that on the first line of the code above, the function `function_name` is defined.  By assigning `function(argument_1, argument_2)` to `function_name` we are declaring a new function called `function_name`, which takes two arguments `argument_1` and `argument_2`.  
...
Let's highlight this point with a basic example using a function that takes one number and raises it to a specified power:
...
In the function declaration above, we've supplied `exp_val` to a default value of 1.  If the user supplies a value for `exp_val` in their function call, then the user supplied value will be used, otherwise the default value will be applied.  Let's illustrate this below.  
