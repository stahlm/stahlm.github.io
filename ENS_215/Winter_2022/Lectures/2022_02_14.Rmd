...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width= 8.5, fig.height= 3}
#library(gridExtra)
library(patchwork)
...
dist_norm <- tibble(x = q_vec, y = dnorm(q_vec, mean = 0, sd = 2), dist = "Normal")
...
Let's create a theoretical Q-Q plot.  In this plot we will compare the distribution of the precipitation data for our state of interest against the theoretical quantiles for a standard **normal distribution**.  
```{r message=FALSE}
...
ggplot(met_data, aes(sample = Wind_m_s)) + 
  ...
  geom_qq_line(distribution = qweibull, dparams = list(shape = shape_fact), color = "blue") +
  theme_classic()
```
