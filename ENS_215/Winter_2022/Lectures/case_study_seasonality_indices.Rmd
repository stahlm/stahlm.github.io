...
You'll now see in your environment pane that you have a function called `compute_seasonality_indices()`.  This is the function we will use to compute the SI and SC indices.  If you click on the function in your environment pane the source code will pop up.  Go ahead and do this.  
...
Now let's use the `compute_seaonality_indices()` function to compute the indices.  
...
The `compute_seaonality_indices()` function returns a list containing two data frames.  You will just need the 2nd data frame.  Let's use the code below to select just the 2nd data frame which has the results we will need. 
...
df_seasonality_precip %>% 
  ggplot(aes(x = SC, y = SI, group = group_name)) +
  
  geom_segment(aes(y = 0, xend = SC, yend = SI),
               size = 0.75, arrow = arrow(length = unit(0.1, "npc")), color = "blue" ) +
