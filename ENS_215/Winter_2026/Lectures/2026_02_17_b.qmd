...
We are going to use the [`terra`](https://rspatial.github.io/terra/index.html) package today. The `terra` package is an R package for spatial analysis, with a primary focus on analyzing raster data.  


Now, let's load in the packages that we will use today.  

::: {.callout-note} 
While we will be using the `terra` package throughout our analysis today we will not load in the **terra** package.  Instead when we want to use a function from the `terra` package we will use the double-colon notation, for example, `terra::plot()` allows us to call the `plot()` function from the `terra` package.  The reason we do this is because a number of the functions in the `terra` package has names that are identical to functions in the `tidyverse`packages. This can create conflicts/issues with your code and we can avoid these issues by calling `terra` functions using the double-colon notation.  
:::
...
Note: If you haven't already done so, then you will need to first install both the `elevatr` and the `tigris` packages.  
...
Just to highlight how the raster resolution makes a difference, below I am showing the raster for Schenectady County where `z = 6` (low resolution).  You can see it has much less detail than the raster above where `z = 12` (high resolution).  
...
You should also install the `rnaturalearthhires` package.  
...
Once you have the remotes package installed (which you should have done earlier in today's lecture), you should then install the `climateR` package by running the following code **in your console**.  
...
Let's go ahead and get the gridded climate monthlies of precipitation for the country of Switzerland.  

First, we will use the `ne_countries()` function from the `rnaturalearth` package to get the border of the country of Switzerland.  
...
::: {.callout-important}
The server that provides TerraClim data was out of service on 18-Feb-2026.  I had been working on 17-Feb-2026, so it is not clear if this is a short-term issue and will be resolved shortly or if this may be a longer-term issue.  We can try this code in class on 19-Feb-2026, however if it doesn't work then we will skip it and get monthly precipitation data from the CHIRPS dataset.  
...
::: {.callout-important}
If the TerraClim code above didn't work, then run the code block directly below.  If the `getTerraClim()` code above worked, then skip the code block directly below.

:::

```{r}
## Run this code if the getTerraClim() above did not work.  Other
...
If you were able to get the TerraClim data, then uncomment the first line of the code block below.  If you got CHIRPS data, then uncomment the second line of code in the code block below.   
...
Pretty cool!  We have a faceted map showing the monthly precipitation for the year 2021 for Switzerland.  You could easily modify the above code to obtain precipitation (or other meteorological data) for a different area of interest.  
...
Now let's make a map to confirm that we did what we had expected to do.  Note that within the `switzerland_tot_prcp` object, the default layer name is `sum`, and thus we need to refer to that layer in the code below.  

```{r}
switzerland_tot_prcp$sum %>% 
  tm_shape() +
...
```{r eval = F, echo= F}
loc_df <- data.frame(long = -73.902778, lat = 44.365833)
loc_df <- st_as_sf(x = loc_df, coords = c("long", "lat"))

st_crs(loc_df) <- 4326

loc_df <- st_buffer(loc_df, dist = 7500)

raster_elev_mt <- get_elev_raster(loc_df, z = 12)
```

```{r echo = F, eval = F}
terra::plot(raster_elev_mt)
```

```{r echo = F, eval = F}
r_slope <- terra::terrain(x = raster_elev_mt, "slope", unit = "degrees")

terra::plot(r_slope)
```
```{r eval = F, echo = F}
r_TRI <- terra::terrain(x = raster_elev_mt, "TRI", unit = "degrees")
