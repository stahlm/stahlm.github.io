...
> **Important:** Before proceeding you will need to [download this zipfile]() from our class site.  This folder has the necessary data for today's lecture.  You should then unzip the folder to a sub-folder in your "Lectures" directory.  Make sure to call this sub-folder "Spatial_data".  
...
You will often want to clip (crop) as raster to a specified area.  For instance you might want to take the global elevation raster that we've been using and clip it to just the United States.  To clip (crop) a raster you will need to specify bounds to clip to.  
