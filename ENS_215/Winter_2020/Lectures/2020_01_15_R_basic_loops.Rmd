...
Ok, now let's try out an example a basic example, so that you learn the structure of for loops.
```{r}
for(i in 1:10){
  print(i)
}
...
```{r}
majors_union <- c("Environmental Science","Geology","English","Chemistry","Math","History","Computer Science")
```
...
You can also loop through a list without using an index variable.
```{r}
...
```{r}
library(readr)
Alb_temps <- read_csv("https://stahlm.github.io/ENS_215/Data/Albany_Temperatures_Nov_2018.csv",skip = 3)
```
...
+ Once you've got that working, add a **counting variable** that keeps a count of the number of days with an "Extreme risk of freezing".  Add similar counting variables for each of the other risk categories.  Then after your loop is complete have your code print a statement reporting the number of days in each catergory.  For example, "_Low risk of freezing: X days_".  
...
If you have any remaining time you should continue practicing with conditional programing (if/else) and control flow (for loops, while loops).  Come up with some of your own ideas that you would like to test out.  Also you can look through the **Base R** cheatsheet and make sure you are familiar with the topics presented there.