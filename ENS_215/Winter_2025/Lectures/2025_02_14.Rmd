...
```{r}
raster_temps_diff_reclass <- terra::classify(raster_temps_diff,
                                             reclass_df)
```
...
In many cases we will want to obtain the values of a raster at specified locations/points.  To do this we can use the `extract()` function from the `raster` package.  Let's test this out by extracting the land surface elevation values from our global elevation raster at the location of every capital city.
...
Now that we have an sf object with the world capitals, let's use the `raster::extract()` function to extract the elevations at each of those locations.  
...
```{r}
cities_world_sf$elev__m <- extract(raster_world_elev, cities_world_sf)
```
