...
Many environmental processes/conditions (e.g., precipitation, temperature, vegetation abundance,...) experience distinct seasonal cycles.  Understanding the characteristics/dynamics of these seasonal cycles can shed light on the underlying processes that control these cycles as well as provide useful information for understanding their environmental and societal implications.  For example, seasonal variations in precipitation can have many important implications ranging from controlling the timing of floods and droughts, influencing soil moisture seasonality, influencing the seasonal growth of vegetation/crops, affecting the transmission of waterborne diseases, and impacting the emergence of insect populations and associated diseases (e.g., Malaria).  
...
>What can we say about the seasonal cycles of precipitation in Boston and In San Francisco?  How would you describe these cycles?  What features stand out?  How do the cycles between the two locations compare and contrast?  

>Can you think of objective metrics that would quantitatively capture the features you observe?   
...
The Seasonal Concentration (SC) indicates the time of year that the precipitation is most focused.  For San Francisco we see that $SC = 20$, indicating precipitation is focused around January.  Boston has a $SC = 264$ indicating precipitation most focused around August.  However, since Boston has its precipitation uniformly distributed throughout the year (i.e., SI is low), then the SC value is not as informative as it is for San Francisco (which has a large SI value).  
...
Now we are going to compute seasonality indices (SI and SC) for each US state using the method introduced above.  First let's load in monthly precipitation data for all of the states in the US.  You'll recall working with this data, which comes from NOAA, in some prior class exercises.  
...
Take a look at the dataset to refresh your memory of what it contains.  You'll see that it has monthly values for precipitation for each state and each year from 1895 through 2024.  
...
FYI, if you want you can also download the R file from my [github site](https://stahlm.github.io/functions/compute_seasonality_indices.R).  
...
Let's remove Alaska and Hawaii since the borders dataset we downloaded doesn't have those states.  
...
Let's go ahead and download streamflow data for the Mohawk River at Freeman's Bridge in Schenectady (USGS 01354500).  I have already obtained the data using the USGS `dataRetrieval` package, which we've used in past classes.  The data is stored on my Github site.  Let's go ahead and load it in.
...
We are going to obtain precipitation data from Albany International Airport (accessed through NOAA's Global Summary of the Day using the `GSODR` package).  I've already obtained this data and it is stored on my Github site.  Let's go ahead and load in this data.  
...
You'll see in the `met_df` data frame that we have daily meteorological data for Albany International Airport for 2014-2024.  Let's go ahead and compute the monthly means so that we can use our `compute_seasonality_indices()` function to calculate SI and SC on the precipitation data.  
