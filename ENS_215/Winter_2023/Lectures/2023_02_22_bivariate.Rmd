...
The `openair` package has a function called `calendarPlot()` that allows you to easily make calendar plots.  

First you'll need to install the `openair` package and then load it in.
...
To make a `calendarPlot` you need to specify the dataset (in this case our `flow` data) as well as the variable to color each day by and also the year of interest.  Note that we specify the data to color the days by using the `pollutant = ` argument.  This argument is called `pollutant` because this package was made by a group of researchers who were using it to analyze and visualize air quality data -- nonetheless we can apply it to any daily time-series data that we want to.  
...
+ Test out `calendarPlot()` with some other years.  
+ Make a calendar plot that shows the average flow for each day of the year (i.e. Jan 1 would show the average flow for all Jan 1 on record, Jan 2 for all Jan 2 on record,...).  You could also make a similar plot for the maximum flows observed on each day.  Examine the results and think about the implications -- discuss your findings with your neighbors.  
...
```{r fig.height= 4.5, echo=FALSE, eval=FALSE}
